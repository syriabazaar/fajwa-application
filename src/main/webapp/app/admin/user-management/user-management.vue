      ...
      <table class="table table-striped" aria-describedby="Users">
        ...
        <tbody v-if="users">
          <tr v-for="user in users" :key="user.id" :id="user.login">
            ...
            <td>
              <button class="btn btn-danger btn-sm deactivated" @click="setActive(user, true)" v-if="!user.activated">Deactivated</button>
              <button
      ...
      <b-modal ref="removeUser" id="removeUser" title="Confirm delete operation" @ok="deleteUser()">
        ...
        <template #modal-footer>
          <div>
            ...
            <button type="button" class="btn btn-primary" id="confirm-delete-user" @click="deleteUser()">Delete</button>
          </div>
        </template>
      </b-modal>
    </div>
